...
const { formatCloudinaryFile } = require('../utils/cloudinaryHelper');
...
const createTask = asyncHandler(async (req, res) => {
  ...
  const task = await Task.create({
    ...
    documents: req.files ? req.files.map(file => formatCloudinaryFile(file)) : []
  });
...
const updateTask = asyncHandler(async (req, res) => {
  ...
  if (req.user.role !== 'admin') {
    console.log('Update Task - Unauthorized: User is not admin');
    res.status(403);
    throw new Error('Not authorized to update this task. Only administrators can edit tasks.');
  }
  ...
  if (req.body.removedFiles) {
    try {
      ...
      const documentsToRemove = documents.filter(doc => removedFileIds.includes(doc._id.toString()));
      ...
      for (const doc of documentsToRemove) {
        if (doc.cloudinaryId) {
          try {
            ...
            console.log('Update Task - Removed file from Cloudinary:', doc.cloudinaryId);
          } catch (err) {
      ...
      documents = documents.filter(doc => !removedFileIds.includes(doc._id.toString()));
    } catch (err) {
  ...
  if (req.files && req.files.length > 0) {
    ...
    const newDocuments = req.files.map(file => formatCloudinaryFile(file));
    ...
    if (documents.length > 3) {
      // Remove excess files from Cloudinary
      for (let i = 3; i < documents.length; i++) {
        if (documents[i].cloudinaryId) {
          try {
            await cloudinary.uploader.destroy(documents[i].cloudinaryId, { resource_type: 'raw' });
            console.log('Update Task - Removed excess file from Cloudinary:', documents[i].cloudinaryId);
          } catch (err) {
            console.error('Error removing file from Cloudinary:', err);
          }
        }
  ...
  if (task.documents && task.documents.length > 0) {
    for (const doc of task.documents) {
      if (doc.cloudinaryId) {
        try {
          await cloudinary.uploader.destroy(doc.cloudinaryId, { resource_type: 'raw' });
          console.log('Delete Task - Removed file from Cloudinary:', doc.cloudinaryId);
        } catch (err) {
          console.error('Error removing file from Cloudinary:', err);
        }
      }
...
const updateTaskStatus = asyncHandler(async (req, res) => {
  ...
  // Check if user is authorized to update this task status
  if (
    req.user.role !== 'admin' && 
    task.assignedTo._id.toString() !== req.user._id.toString() &&
    task.createdBy._id.toString() !== req.user._id.toString()
